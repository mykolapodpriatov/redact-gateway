...
- **Detectors (pluggable `Detector` interface):**
  ...
  - `regex-pii` — runs email/card/SSN-style regexes over a pluggable `OCR` interface (the default OCR is a no-op, so it finds nothing until a real OCR adapter is supplied — see Scope). The `card` category is additionally **Luhn-validated**: a matched 13-19 digit run is only masked when its mod-10 checksum passes, so order numbers, IMEIs, and tracking/serial IDs no longer trigger a false-positive redaction. Validation is a per-pattern `Validate func(string) bool` hook, so `email`/`ssn` are unaffected and custom patterns can opt in,
